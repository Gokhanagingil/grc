...
if ([string]::IsNullOrWhiteSpace($Email2)) { $Email2 = $Email1 }
if ([string]::IsNullOrWhiteSpace($Pass2)) { $Pass2 = $Pass1 }
...
function Try-Health {
  ...
  foreach ($p in $paths) {
    try {
      $r = Invoke-WebRequest -Uri ($Base + $p) -UseBasicParsing -TimeoutSec 5 -Method GET
...
function Try-Login([string]$email,[string]$pass,[string]$label) {
  try {
    ...
    $r = Invoke-WebRequest -Uri ($Base + "/api/v2/auth/login") -Headers @{ $TenantHeader = $TenantId; "Content-Type"="application/json" } -Body $body -Method POST -UseBasicParsing -TimeoutSec 8
...
$okH = Try-Health
if (-not $okH) {
  Write-Host "login_usr1 SKIP (health fail)"
  Write-Host "login_usr2 SKIP (health fail)"
  exit 0
}
