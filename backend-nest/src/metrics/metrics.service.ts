...
export class MetricsService {
   ...
   */
  recordError(method: string, path: string, statusCode: number): void {
    const routeKey = `${method} ${path}`;
  ...
  getActiveHandlesCount(): number {
    // @ts-ignore - _getActiveHandles is a private Node.js API
  ...
  toPrometheusFormat(): string {
    ...
    // Uptime
    lines.push('# HELP uptime_seconds The number of seconds the application has been running');
    ...
    // Active handles
    lines.push('# HELP active_handles_count Number of active handles in the event loop');
    ...
    // Average latency
    lines.push('# HELP http_request_latency_ms_avg Average request latency in milliseconds');
    ...
    for (const [route, metrics] of this.routeMetrics) {
      ...
      lines.push(`http_request_count{method="${method}",route="${sanitizedPath}"} ${metrics.count}`);
    }
    lines.push('');

    lines.push('# HELP http_request_latency_ms_max Maximum request latency per route in milliseconds');
    lines.push('# TYPE http_request_latency_ms_max gauge');
    for (const [route, metrics] of this.routeMetrics) {
      const [method, path] = route.split(' ');
      const sanitizedPath = path.replace(/"/g, '\\"');
      lines.push(`http_request_latency_ms_max{method="${method}",route="${sanitizedPath}"} ${metrics.maxLatencyMs}`);
    }
    lines.push('');

    lines.push('# HELP http_request_latency_ms_avg Average request latency per route in milliseconds');
    lines.push('# TYPE http_request_latency_ms_avg gauge');
    for (const [route, metrics] of this.routeMetrics) {
      const [method, path] = route.split(' ');
      const sanitizedPath = path.replace(/"/g, '\\"');
      const avgLatency = metrics.count > 0 ? Math.round(metrics.totalLatencyMs / metrics.count * 100) / 100 : 0;
      lines.push(`http_request_latency_ms_avg{method="${method}",route="${sanitizedPath}"} ${avgLatency}`);
    }
    lines.push('');
    ...
    for (const [route, metrics] of this.routeMetrics) {
      const [method, path] = route.split(' ');
      const sanitizedPath = path.replace(/"/g, '\\"');
      lines.push(`http_error_count{method="${method}",route="${sanitizedPath}"} ${metrics.errorCount}`);
    }
  ...
  toJson(): Record<string, unknown> {
    ...
    for (const [route, metrics] of this.routeMetrics) {
      routes[route] = {
        count: metrics.count,
        avgLatencyMs: metrics.count > 0 ? Math.round(metrics.totalLatencyMs / metrics.count * 100) / 100 : 0,
