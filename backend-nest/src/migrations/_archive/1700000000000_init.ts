...
export class Init1700000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasPolicies) {
      ...
      const ensure = async (c: TableColumn) => {
        if (!cols?.columns?.find((x) => x.name === c.name))
          await queryRunner.addColumn('policies', c);
      };
      await ensure(
        new TableColumn({
          name: 'deletedAt',
      ...
      const codeIdx = cols?.indices?.find(
        (i) => i.columnNames.join(',') === 'code',
      );
      if (!codeIdx)
        await queryRunner.createIndex(
          'policies',
    ...
    if (!hasGov) {
      ...
      await queryRunner.createIndex(
        'gov_policies',
        new TableIndex({ name: 'IDX_gov_title', columnNames: ['title'] }),
      );
      await queryRunner.createIndex(
        'gov_policies',
        new TableIndex({ name: 'IDX_gov_category', columnNames: ['category'] }),
      );
    }
    ...
    if (!hasRisks) {
      ...
      await queryRunner.createIndex(
        'risks',
        new TableIndex({ name: 'IDX_risk_title', columnNames: ['title'] }),
      );
      await queryRunner.createIndex(
        'risks',
        new TableIndex({
      ...
      await queryRunner.createIndex(
        'risks',
        new TableIndex({ name: 'IDX_risk_status', columnNames: ['status'] }),
      );
    }
    ...
    if (!hasReq) {
      ...
      await queryRunner.createIndex(
        'requirements',
        new TableIndex({ name: 'IDX_req_title', columnNames: ['title'] }),
      );
      await queryRunner.createIndex(
        'requirements',
        new TableIndex({
          name: 'IDX_req_regulation',
          columnNames: ['regulation'],
        }),
      );
      await queryRunner.createIndex(
        'requirements',
        new TableIndex({ name: 'IDX_req_status', columnNames: ['status'] }),
      );
    }

    // Optional trigram indexes for ILIKE performance
    await queryRunner.query(
      'CREATE INDEX IF NOT EXISTS "GIN_gov_title_trgm" ON "gov_policies" USING GIN ("title" gin_trgm_ops);',
    );
    await queryRunner.query(
      'CREATE INDEX IF NOT EXISTS "GIN_risk_title_trgm" ON "risks" USING GIN ("title" gin_trgm_ops);',
    );
    await queryRunner.query(
      'CREATE INDEX IF NOT EXISTS "GIN_req_title_trgm" ON "requirements" USING GIN ("title" gin_trgm_ops);',
    );
  }
