...
export const API_PATHS = {
  ...
  ITSM: {
    ...
    CHANGES: {
      ...
      // Change Task endpoints
      TASKS: (changeId: string) => `/grc/itsm/changes/${changeId}/tasks`,
    ...
    // Change Template endpoints
    CHANGE_TEMPLATES: {
      LIST: '/grc/itsm/change-templates',
      CREATE: '/grc/itsm/change-templates',
      GET: (id: string) => `/grc/itsm/change-templates/${id}`,
...
// ========== Change Task Types ==========
export type ChangeTaskStatus = 'DRAFT' | 'OPEN' | 'IN_PROGRESS' | 'PENDING' | 'COMPLETED' | 'FAILED' | 'SKIPPED' | 'CANCELLED';
export type ChangeTaskType = 'IMPLEMENTATION' | 'VALIDATION' | 'BACKOUT' | 'PRE_CHECK' | 'REVIEW' | 'APPROVAL_GATE' | 'OTHER';
export type ChangeTaskPriority = 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';

export interface ChangeTaskReadiness {
  isReady: boolean;
...
export interface ChangeTaskData {
  ...
  createdAt: string;
  updatedAt: string;
  readiness?: ChangeTaskReadiness;
}

export interface CreateChangeTaskDto {
  title: string;
  description?: string;
  status?: ChangeTaskStatus;
  taskType?: ChangeTaskType;
...
export interface UpdateChangeTaskDto {
  title?: string;
  description?: string;
  status?: ChangeTaskStatus;
  taskType?: ChangeTaskType;
...
export interface ChangeTaskDependencyData {
  id: string;
  predecessorTaskId: string;
  successorTaskId: string;
}

export interface ChangeTemplateData {
  id: string;
  tenantId: string;
  name: string;
  code: string;
  ...
  isGlobal: boolean;
  version: number;
  tasks?: ChangeTemplateTaskData[];
  dependencies?: ChangeTemplateDependencyData[];
  createdAt: string;
  updatedAt: string;
}

export interface ChangeTemplateTaskData {
  id: string;
  taskKey: string;
  title: string;
  description?: string | null;
...
export interface ChangeTemplateDependencyData {
  id: string;
...
export const itsmApi = {
  ...
  changes: {
    ...
    // Change Task endpoints
    listTasks: (changeId: string, params?: { page?: number; pageSize?: number; status?: string; taskType?: string; search?: string }) => {
      const searchParams = new URLSearchParams();
      if (params?.page) searchParams.set('page', String(params.page));
      if (params?.pageSize) searchParams.set('pageSize', String(params.pageSize));
      ...
      if (params?.search) searchParams.set('search', params.search);
      const queryString = searchParams.toString();
      return api.get(`${API_PATHS.ITSM.CHANGES.TASKS(changeId)}${queryString ? `?${queryString}` : ''}`);
    },
    createTask: (changeId: string, data: CreateChangeTaskDto) =>
      api.post(API_PATHS.ITSM.CHANGES.TASKS(changeId), data),
    getTask: (changeId: string, taskId: string) =>
      api.get(API_PATHS.ITSM.CHANGES.TASK(changeId, taskId)),
    updateTask: (changeId: string, taskId: string, data: UpdateChangeTaskDto) =>
      api.patch(API_PATHS.ITSM.CHANGES.TASK(changeId, taskId), data),
    deleteTask: (changeId: string, taskId: string) =>
      api.delete(API_PATHS.ITSM.CHANGES.TASK(changeId, taskId)),
    getTaskSummary: (changeId: string) =>
      api.get(API_PATHS.ITSM.CHANGES.TASK_SUMMARY(changeId)),
    listTaskDependencies: (changeId: string) =>
      api.get(API_PATHS.ITSM.CHANGES.TASK_DEPENDENCIES(changeId)),
    addTaskDependency: (changeId: string, data: { predecessorTaskId: string; successorTaskId: string }) =>
  ...
  changeTemplates: {
    list: (params?: { page?: number; pageSize?: number; search?: string; isActive?: boolean }) => {
      ...
      const queryString = searchParams.toString();
      return api.get(`${API_PATHS.ITSM.CHANGE_TEMPLATES.LIST}${queryString ? `?${queryString}` : ''}`);
    },
    get: (id: string) => api.get(API_PATHS.ITSM.CHANGE_TEMPLATES.GET(id)),
    create: (data: Record<string, unknown>) => api.post(API_PATHS.ITSM.CHANGE_TEMPLATES.CREATE, data),
    update: (id: string, data: Record<string, unknown>) => api.patch(API_PATHS.ITSM.CHANGE_TEMPLATES.UPDATE(id), data),
    delete: (id: string) => api.delete(API_PATHS.ITSM.CHANGE_TEMPLATES.DELETE(id)),
    apply: (data: { templateId: string; changeId: string; force?: boolean }) =>
      api.post(API_PATHS.ITSM.CHANGE_TEMPLATES.APPLY, data),
  },
